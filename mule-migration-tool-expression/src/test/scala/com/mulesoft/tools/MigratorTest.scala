...
class MigratorTest extends FlatSpec with Matchers {
  "Migrator" should "migrate a map" in {
    Migrator.migrate(" [ \"a\" : [1,2], \"b\" : 666  ]   ") shouldBe "%dw 2.0\n---\n{\n  \"a\": [1, 2],\n  \"b\": 666\n}"
  }

  "Migrator" should "migrate a map with identifiers" in {
    Migrator.migrate("['user-agent': 'Mule 3.8.0']") shouldBe "%dw 2.0\n---\n{\n  \"user-agent\": 'Mule 3.8.0'\n}"
  }

  it should "migrate a list of lists" in {
    Migrator.migrate("[[3,5,7],[]]") shouldBe "%dw 2.0\n---\n[\n  [3, 5, 7],\n  []\n]"
  }

  it should "migrate an empty list" in {
    Migrator.migrate("[]") shouldBe "%dw 2.0\n---\n[]"
  }

  it should "migrate an number" in {
    Migrator.migrate("1.2") shouldBe "%dw 2.0\n---\n1.2"
  }
  ...
  it should "migrate a dot expression" in {
    Migrator.migrate("payload.foo") shouldBe "%dw 2.0\n---\npayload.foo"
  }

  it should "migrate a subscript expression" in {
    Migrator.migrate("message.inboundProperties['http.query.params']") shouldBe "%dw 2.0\n---\nmessage.inboundProperties['http.query.params']"
  }

  it should "migrate a subscript expression without binding context" in {
    Migrator.migrate("lala.pepe") shouldBe "%dw 2.0\n---\nvars.lala.pepe"
  }

  it should "migrate a not equals expression" in {
    Migrator.migrate("payload != null") shouldBe "%dw 2.0\n---\npayload != null"
  }
  ...
  it should "migrate a less than expression" in {
    Migrator.migrate("payload.bla < 5 + 3") shouldBe "%dw 2.0\n---\npayload.bla < 5 + 3"
  }

  it should "migrate a greater than expression" in {
    Migrator.migrate("4 + 7 > 5 + 3") shouldBe "%dw 2.0\n---\n4 + 7 > 5 + 3"
  }

  it should "migrate a greater than or equals expression" in {
    Migrator.migrate("4 >= 2 + 2") shouldBe "%dw 2.0\n---\n4 >= 2 + 2"
  }

  it should "migrate a smaller than or equals expression" in {
    Migrator.migrate("2 + 2 <= 4") shouldBe "%dw 2.0\n---\n2 + 2 <= 4"
  }

  it should "migrate an and expression" in {
    Migrator.migrate("true && true == true") shouldBe "%dw 2.0\n---\ntrue and true == true"
  }
  ...
  it should "migrate multiple multiplication" in {
    Migrator.migrate("1*2*3") shouldBe "%dw 2.0\n---\n1 * 2 * 3"
  }
  ...
  it should "migrate a generic constructor with multiple arguments" in {
    Migrator.migrate("new com.lala.tools.Pepe('a',1)") shouldBe "%dw 2.0\n---\njava!com::lala::tools::Pepe::new('a', 1)"
  }
